...
jest.mock('../../modules/opa-protection', () => ({ claimPendingOpaProtectionTrigger: jest.fn(), acknowledgeClaimedOpaProtectionTrigger: jest.fn(), releasePendingOpaProtectionTrigger: jest.fn() }));
...
describe('headless VOICE location through real worker and processors', () => {
  ...
  it('uses real available location and exact ACK', async () => {
    ...
    expect(post.mock.calls[0][1]).toMatchObject({ triggerType: 'VOICE', mode: 'SILENT', latitude: 6.5, longitude: 3.3 });
  ...
  it.each(['denied', 'gps failure', 'permission read failure'])('activates without fabricated coordinates on %s and drains following SOS', async (failure) => {
    ...
    expect(post.mock.calls[0][1]).toMatchObject({ triggerType: 'VOICE', mode: 'SILENT', userConfirmed: false });
  ...
  it('preserves SOS no-location retry without calling activation', async () => {
    claim.mockReset().mockResolvedValue({ ...voice, type: 'SOS_BUTTON' });
    (Location.getForegroundPermissionsAsync as jest.Mock).mockResolvedValue({ status: 'denied' });
    await runHeadlessProtectionWorker();
