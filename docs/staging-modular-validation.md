...
The operator-owned supervisor invokes `node apps/api/scripts/staging-gate-runner.cjs` with a JSON object on stdin, never command-line credentials. Fields are `policy` (Ed25519 envelope), `runtimeUrl` (protected staging source, kept in memory), and `vaultToken` (short-lived staging migration-identity token). Do not store this input or print it. No default live execution or auto-retry exists.
...
Prepare an exact clean branch checkout, npm ci, pinned Node 22.23.2 / npm 10.9.8 / Prisma and client 6.19.3, generated dependencies and reviewed OS isolation before invocation. Source SHA, clean checkout, branch, versions, physical private address and DNS are rechecked. Generate/validate/build rerun before database validation. A signed policy refresh is required if the prior policy expires. Cleanup remains permitted after expiry; another gate does not begin.
