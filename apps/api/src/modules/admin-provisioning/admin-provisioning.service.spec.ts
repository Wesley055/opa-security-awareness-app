...
describe("AdminProvisioningService", () => {
  it("fails closed for the retired staff token provisioning methods", async () => {
    const service = new AdminProvisioningService({} as never);
    await expect(
      service.createOperatorSeat("admin", {} as never),
    ).rejects.toThrow("Use durable platform enrollment invitations.");
    await expect(
      service.createFacilityAdminSeat("admin", {} as never),
    ).rejects.toThrow("Use durable platform enrollment invitations.");
  });
  const prisma = {
    $transaction: jest.fn(),
