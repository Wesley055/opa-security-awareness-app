...
module.exports = function withCanonicalBrand(config) {
  return withDangerousMod(config, ['android', async config => {
    ...
    if (digest(fs.readFileSync(path.join(repository, register.source))) !== register.sourceSha256) throw new Error('Canonical brand master hash mismatch');
    ...
    for (const asset of expected) {
      if (digest(fs.readFileSync(path.join(repository, asset.path))) !== asset.sha256) throw new Error('Canonical native asset missing or changed: ' + asset.path);
    }
    for (const directory of fs.readdirSync(source)) {
      ...
      for (const filename of fs.readdirSync(path.join(source,directory))) {
        ...
        if (!asset || digest(fs.readFileSync(path.join(source,directory,filename))) !== asset.sha256) throw new Error('Canonical native asset hash mismatch: ' + relative);
