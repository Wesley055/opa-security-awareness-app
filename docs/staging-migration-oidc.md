...
## Temporary push auth-only proof

The exact path ops/staging/oidc-auth-trigger.json on integration/institutional-security enables a separate protected staging auth-only job. The manual job explicitly rejects push events. The push job contains no migration command or migration-helper invocation. Its mode and migration flag are fixed, independently of dispatch inputs.

Before Azure login, its guard checks the original push event, exact repository/ref, environment, run identifiers, non-forced/non-deleted push, trigger parent SHA, and auth-only trigger. Both the operator-host RAM-backed lease and the signed staging policy must bind the actual GitHub commit SHA. The policy is signed and published after the commit to avoid a self-referential commit hash. The GitHub OIDC claims must additionally bind the staging environment, repository, ref, SHA and workflow. Credentials remain in memory.

For this temporary path, the operator starts a fresh single-job runner for the trigger lease and watches the push run instead of dispatching the manual workflow. The same private /32 lease, protected environment review, container restrictions and cleanup requirements apply. No default-branch change is needed. Trigger edits alone cannot request migration. The mechanism can remain until the separately approved migration is complete; this auth-only proof does not authorize migration or deployment.
