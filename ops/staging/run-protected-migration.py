...
def observed_run(command, **kwargs):
 ...
 label='docker.'+command[1] if command[0]==DOCKER else 'git' if executable=='git' else 'azure-cli' if command[0]==AZ else 'custodian' if command[0]==NODE else 'vpn-address'
 return DIAG.call(label,label,subprocess.run,command,**kwargs)

def observed_request(method,url,**kwargs):
 label='azure.management' if url.startswith('https://management.azure.com/') else 'github.management'
 return DIAG.call(label,method.lower()+'.'+label,requests.request,method,url,**kwargs)

def observed_get(url,**kwargs):
 return observed_request('GET',url,**kwargs)
...
def execute():
 ...
 DIAG.file=ROOT/'artifacts/staging-migration-runs'/(str(args.run_id)+'-supervisor-'+datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%S%f')+'.json')
 require(re.fullmatch('[a-f0-9]{40}',args.sha),'SHA_FORMAT')
 require(observed_run(['git','-c','safe.directory=C:/Projects/OPA','-C',str(ROOT),'diff','--quiet','HEAD','--','apps/api/scripts/staging-validation-custodian.cjs','apps/api/scripts/staging-database-verifier.cjs','ops/staging/run-protected-migration.py','ops/staging/runner_runtime.py','ops/staging/runner/image.lock.json','ops/staging/runner/Dockerfile','ops/staging/runner/verify-toolchain.cjs','ops/staging/runner/signal-cleanup.cjs']).returncode==0,'HOST_EXECUTABLE_CHANGED')
 require(git('rev-parse','HEAD')==args.sha and git('branch','--show-current')==BRANCH,'LOCAL_SHA')
 image_lock=json.loads((ROOT/'ops/staging/runner/image.lock.json').read_text())
 require(args.image==image_lock['imageId'] and re.fullmatch(r'sha256:[a-f0-9]{64}',args.image),'PINNED_IMAGE_REQUIRED')
 image_id=observed_run([DOCKER,'image','inspect','--format','{{.Id}}',args.image],capture_output=True,text=True,timeout=20)
 require(image_id.returncode==0 and image_id.stdout.strip()==args.image,'LOCAL_IMAGE_DIGEST')
 toolcheck=observed_run([DOCKER,'run','--rm','--network','none','--read-only','--cap-drop','ALL','--security-opt','no-new-privileges','--entrypoint','/home/runner/externals/node24/bin/node',args.image,'/opt/opa/verify-toolchain.cjs'],capture_output=True,text=True,timeout=45)
 require(toolcheck.returncode==0,'PINNED_TOOLCHAIN_REQUIRED')
 tools=json.loads(toolcheck.stdout)
 require(tools=={'node':{'path':'/opt/opa/node22/bin/node','version':'v22.23.2'},'npm':{'path':'/opt/opa/node22/bin/npm','version':'10.9.8'},'npx':{'path':'/opt/opa/node22/bin/npx','version':'10.9.8'}},'PINNED_TOOLCHAIN_METADATA')
 DIAG.versions=tools;DIAG.save()
 ...
 c=observed_run(['git','-c','safe.directory=C:/Projects/OPA','-C',str(ROOT),'-c','credential.interactive=false','credential','fill'],input='protocol=https\nhost=github.com\n\n',capture_output=True,text=True,env=env,timeout=20)
 ...
 def gh(method,suffix,body=None,allowed=(200,201,204)):
  x=observed_request(method,'https://api.github.com/repos/'+REPO+suffix,headers=headers,json=body,timeout=30,allow_redirects=False);require(x.status_code in allowed,'GITHUB_METADATA');return x
 ...
 if args.execute:
  x=observed_get(f'https://api.github.com/repositories/{repo["id"]}/environments/staging/variables/OPA_STAGING_MIGRATION_AUTHORIZATION',headers=headers,timeout=30);require(x.status_code==200,'MIGRATION_AUTHORIZATION_MISSING');a=json.loads(x.json()['value'])
 ...
 def arm(method,resource,body=None,allowed=(200,201,202,204)):
  ...
  x=observed_request(method,'https://management.azure.com'+resource,headers={'Authorization':'Bearer '+credential.get_token('https://management.azure.com/.default').token},json=body,timeout=45,allow_redirects=False);require(x.status_code in allowed,'ARM_OPERATION');return x
 ...
 roles=observed_run([AZ,'role','assignment','list','--assignee',PRINCIPAL,'--all','--include-inherited','--output','json','--only-show-errors'],capture_output=True,text=True,timeout=60);require(roles.returncode==0,'IAM_QUERY');roles=json.loads(roles.stdout)
 ...
 address=observed_run(['powershell','-NoProfile','-Command',"@(Get-NetIPAddress -AddressFamily IPv4 | Where-Object IPAddress -like '172.27.240.*' | Select-Object -ExpandProperty IPAddress) | ConvertTo-Json -Compress"],capture_output=True,text=True,timeout=20)
 ...
 audit={'runId':args.run_id,'sha':args.sha,'execute':args.execute,'lease':lease,'networkCleanup':[],'testDatabaseRequested':False,'testDatabaseCleanup':not args.execute,'temporaryIamRemoved':True,'containerRemoved':False,'runnerDeregistered':False};rules=[];container=None;runner_id=None;iam=None;container_requested=False
 ...
 def custody(action):
  ...
  result=observed_run([NODE,str(ROOT/'apps/api/scripts/staging-validation-custodian.cjs')],input=json.dumps(data),text=True,capture_output=True,timeout=180);data.clear()
 ...
 def publish(file,data):
  ...
  result=observed_run([DOCKER,'exec','-i',name,'/runner/externals/node24/bin/node','-e',code,file],input=json.dumps(data),text=True,capture_output=True,timeout=20);require(result.returncode==0,'RUNNER_RESPONSE')
 try:
  ...
  entry='/home/runner/externals/node24/bin/node /opt/opa/verify-toolchain.cjs >/dev/null && cp -a /home/runner/. /runner/ && cd /runner && exec /home/runner/externals/node24/bin/node -e "$1"'
  ...
  container=DIAG.call('runner.start','docker.run',subprocess.Popen,cmd,stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True)
  def discard(stream):
  ...
  DIAG.last_stage='runner.registration.stdin'
  ...
  while time.monotonic()<deadline:
   ...
   if args.execute and container.poll() is None:
    ...
    request=observed_run([DOCKER,'exec',name,'/runner/externals/node24/bin/node','-e',code],capture_output=True,text=True,timeout=20)
  ...
  if container is not None:
   status=container.poll();audit['containerProcess']={'exitCode':status,'signal':-status if isinstance(status,int) and status<0 else None}
  if container_requested:
   try:
    snapshot_code="const fs=require('fs'),os=require('os');const d=fs.statfsSync('/runner');console.log(JSON.stringify({diskFreeBytes:d.bavail*d.bsize,memoryAvailableBytes:os.freemem(),memoryTotalBytes:os.totalmem()}));"
    snapshot=observed_run([DOCKER,'exec',name,'/home/runner/externals/node24/bin/node','-e',snapshot_code],capture_output=True,text=True,timeout=20)
    if snapshot.returncode==0:
     values=json.loads(snapshot.stdout)
     audit['runnerResources']={k:values[k] for k in ('diskFreeBytes','memoryAvailableBytes','memoryTotalBytes') if isinstance(values.get(k),int) and values[k]>=0}
   except Exception:
    audit['runnerResourcesUnavailable']=True
  def cancel():
   if audit.get('conclusion')!='success':gh('POST','/actions/runs/'+str(args.run_id)+'/cancel',{},allowed=(202,409))
   return True
  runtime.cleanup_step('cancel',cancel,DIAG,outcomes)
  def database_cleanup():
   if args.execute and audit['testDatabaseRequested']:custody('drop')
   audit['testDatabaseCleanup']=True
   return True
  runtime.cleanup_step('database',database_cleanup,DIAG,outcomes)
  def remove_arm(resource):
   arm('DELETE',resource,allowed=(200,202,204,404))
   for attempt in range(15):
    if arm('GET',resource,allowed=(200,404)).status_code==404:return True
    time.sleep(2)
   return False
  def iam_cleanup():
   audit['temporaryIamRemoved']=not iam or remove_arm(iam)
   return audit['temporaryIamRemoved']
  runtime.cleanup_step('iam',iam_cleanup,DIAG,outcomes)
  for index,resource in enumerate(reversed(rules)):
   removed=runtime.cleanup_step('nsg-'+str(index),lambda resource=resource:remove_arm(resource),DIAG,outcomes)
   audit['networkCleanup'].append({'resource':resource,'removed':removed})
  def authorization_cleanup():
   if not args.execute:return True
   suffix='/environments/staging/variables/OPA_STAGING_MIGRATION_AUTHORIZATION'
   current=gh('GET',suffix,allowed=(200,404))
   if current.status_code==404:return True
   auth=json.loads(current.json()['value'])
   require(auth['sha']==args.sha and auth['lease']==leaseid and auth['action']==('VALIDATE_MIGRATED_OPA_STAGING' if trigger['mode']=='validation' else 'MIGRATE_OPA_STAGING'),'AUTHORIZATION_CLEANUP_OWNERSHIP')
   gh('DELETE',suffix,allowed=(204,404))
   for attempt in range(10):
    if gh('GET',suffix,allowed=(200,404)).status_code==404:return True
    time.sleep(2)
   return False
  audit['executionAuthorizationRemoved']=runtime.cleanup_step('authorization',authorization_cleanup,DIAG,outcomes)
  def container_cleanup():
   if not container_requested:
    audit['containerRemoved']=True
    return True
   observed_run([DOCKER,'rm','--force',name],capture_output=True,timeout=30)
   remaining=observed_run([DOCKER,'ps','-aq','--filter','name=^/'+name+'$'],capture_output=True,text=True,timeout=20)
   audit['containerRemoved']=remaining.returncode==0 and not remaining.stdout.strip()
   return audit['containerRemoved']
  runtime.cleanup_step('container',container_cleanup,DIAG,outcomes)
  def registration_cleanup():
   if not container_requested:
    audit['runnerDeregistered']=True
    return True
   matching=[x for x in gh('GET','/actions/runners?per_page=100').json()['runners'] if x['name']==name]
   require(len(matching)<=1,'RUNNER_CLEANUP_AMBIGUOUS')
   if matching:gh('DELETE','/actions/runners/'+str(matching[0]['id']),allowed=(204,404))
   audit['runnerDeregistered']=not any(x['name']==name for x in gh('GET','/actions/runners?per_page=100').json()['runners'])
   return audit['runnerDeregistered']
  runtime.cleanup_step('registration',registration_cleanup,DIAG,outcomes)
  audit['cleanupOutcomes']=outcomes
  save();DIAG.save();print(json.dumps(audit),flush=True)
  require(all(outcomes.values()),'CLEANUP_INCOMPLETE')

if __name__=='__main__':
 try:
  with DIAG.operation('supervisor','protected-validation-supervisor'):
   execute()
 except Exception:
  DIAG.save()
  print(json.dumps({'status':'Stopped','reason':'SUPERVISOR_FAILED','diagnosticStage':DIAG.failure_stage or DIAG.last_stage}))
  sys.exit(1)
