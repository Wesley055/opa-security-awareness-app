...
describe("InvitationDeliveryWorker integration", () => {
  ...
  async function createPendingResident(facilityId: string): Promise<User> {
    return prismaTest.user.create({
      data: {
        ...
        phoneNumber:
          "+234" + String(800000000 + Math.floor(Math.random() * 1e8)),
        passwordHash: null,
        firstName: "Pending",
        lastName: "Resident",
        role: "USER",
  ...
  function buildWorker(send: jest.Mock) {
    return new InvitationDeliveryWorker(
      prismaTest as never,
      { send } as never,
  ...
  it("does not blindly resend legacy SENDING rows without attempt evidence", async () => {
    const facility = await createFacility();
    ...
    expect(recovered.deliveryStatus).toBe("UNKNOWN");
    expect(send).not.toHaveBeenCalled();
